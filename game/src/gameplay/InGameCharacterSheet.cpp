                             ...
                             const char *label2, uint8_t r = 200, uint8_t g = 200, uint8_t b = 200)
{
    ...
    int cx = col;
    for (int i = 0; i < left_rule; ++i) {
        c.drawGlyph(cellX(cx + i), cellY(row), 0x07, r, g, b, 255);
    }
    cx += left_rule;
    drawCellText(c, row, cx, label1, r, g, b);
    cx += l1;
    for (int i = 0; i < mid_rule; ++i) {
        c.drawGlyph(cellX(cx + i), cellY(row), 0x07, r, g, b, 255);
    }
    cx += mid_rule;
    drawCellText(c, row, cx, label2, r, g, b);
    cx += l2;
    for (int i = 0; i < right_rule; ++i) {
        c.drawGlyph(cellX(cx + i), cellY(row), 0x07, r, g, b, 255);
    }
}
                                       ...
                                       const SheetSession *session) const
{
    ...
    if (has_book_) {
        blitBookFrame(c, book_, 11, kPartyPanelBlitX, kPartyPanelBlitY);
    } else {
    ...
    drawBorderIntegratedTextAt(c, kSheetHeaderRow, kSheetHeaderCol, header);
    ...
    drawCellText(c, kSheetFooterRow2, kSheetFooterCol + 20, "'S' Share  'T' Trade  'U' Use", 180, 180, 180);

    if (session && session->status_line[0]) {
        drawBorderIntegratedTextAt(c, kSheetFooterRow1 - 1, kSheetFooterCol, session->status_line, 255, 255, 128);
    }
...
{
    drawRedBorder(c, 1, 1, 38, 21);

    drawCellText(c, 2, 2, "#     Name    Hit Points  Spell Points", 255, 255, 128);
    drawCellText(c, 3, 2, "# Lvl SL AC Age Gems  Food Condition", 255, 255, 128);
    ...
    for (int i = 0; i < launch.party_count && i < 8; ++i) {
        ...
        char line1[64];
        std::snprintf(line1, sizeof(line1), "%d) %-8s %3u/%3u       %3u/%3u", i + 1, name, rec.hp_current,
                      rec.hp_max, rec.sp_current, rec.sp_max);
        drawCellText(c, 4 + i, 2, line1);

        char line2[64];
        std::snprintf(line2, sizeof(line2), "%d  L%u SL%u AC%u Age%u G%u F%u %s", i + 1, rec.level, rec.spell_level,
                      rec.armor_class, rec.age, rec.gems, rec.food, conditionName(rec.condition));
        drawCellText(c, 14 + i, 2, line2, 200, 200, 200);
    }

    drawBorderIntegratedText(c, 22, 1, 38, "( 'ESC' to go back )", 180, 180, 180);
}
