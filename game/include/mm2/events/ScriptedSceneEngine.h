...
private:
    ...
    void blitPegasusOverlay(gfx::ScreenCompositor &c) const;
    void blitGhostOverlay(gfx::ScreenCompositor &c) const;
    void drawGhostPlaceholder(gfx::ScreenCompositor &c) const;

    static bool tryLoadAnm(const char *data_dir, int anm_id, uint8_t **rgba_out, int *w_out, int *h_out);
    ...
    uint8_t *pegasus_rgba_ = nullptr;
    int pegasus_w_ = 0;
    int pegasus_h_ = 0;
    bool has_pegasus_sprite_ = false;

    uint8_t *ghost_rgba_ = nullptr;
    int ghost_w_ = 0;
    int ghost_h_ = 0;
