...
def export_gallery(*, skip_gallery: bool) -> None:
    if skip_gallery:
        ...
        stub.write_text(
            ...
            "</div>\n\n"
            "---\n\n"
            "This export used **`--skip-gallery`** (docs-only). Amiga sprite pages/images "
            "are omitted here; `publish-github-wiki.py` preserves them from the live wiki "
            "when present. To regenerate from game assets:\n\n"
            "```powershell\n"
...
def export_maze_walker(out_root: Path) -> None:
    ...
    walker_page.write_text(
        ...
        "**GitHub Wiki cannot run JavaScript.** Open the walker on GitHub Pages:\n\n"
        f"**[{pages_url}]({pages_url})**\n\n"
        "Controls: **W/S** forward/back, **A/D** turn. Toolbar: **Graphics** "
        "(Amiga / PC CGA / PC EGA) and **Wallset** (Auto / Town / Cavern / Castle). "
        "PC wall art is exported from GOG `.4`/`.16` via `parse_wall_sheet` (packed-u32 tables).\n\n"
